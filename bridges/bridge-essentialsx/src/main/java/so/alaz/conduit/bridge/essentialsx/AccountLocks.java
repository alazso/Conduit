...
/**
 * Per-account serialization for the EssentialsX bridge.
 *
 * <p>EssentialsX exposes no transactional API, so every balance change is a
 * read-modify-write. Because the bridge dispatches those writes onto a
 * multi-threaded async executor, concurrent operations on the same account would
 * otherwise race (lost updates, overdraft). Each operation runs under the
 * account's lock; two-account operations (transfer) acquire both locks in
 * {@link UUID#compareTo(UUID) UUID order} to avoid deadlock.
 *
